...
class _RelayPageState extends State<RelayPage> {
  ...
  late Future<List<ServiceGroup>> _groups = _service.loadAll();

  void _reload() {
    setState(() {
      _groups = _service.loadAll();
    });
  }
  ...
  Future<void> _openEditor([ServiceGroup? group]) async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (context) => RelayEditorPage(group: group),
      ),
    );
...
class _RelayEditorPageState extends State<RelayEditorPage> {
  ...
  void initState() {
    ...
    _port.text = (group?.relayPort ?? 8080).toString();
